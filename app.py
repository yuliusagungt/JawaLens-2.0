...
st.markdown("<p style='text-align: center; color: #8D6E63; margin-bottom: 2rem;'>Transliteration your Javanese Script</p>", unsafe_allow_html=True)
...
if st.session_state.uploaded_image is not None:
    ...
    with col1:
        if st.button("✂️ Crop Image", use_container_width=True):
            st.session_state.show_cropper = True
            st.session_state.final_image = None
            st.rerun()
    
    with col2:
        if st.button("➡️ Process As-Is", use_container_width=True):
            st.session_state.show_cropper = False
...
if st.session_state.final_image is not None:
    ...
    processing_mode = st.radio(
        ...
        format_func=lambda x: {
            "simple": "🎨 Simple Image Processing",
            "javanese": "📜 Javanese Script Transliteration (JawaLens 2.0)"
        }[x],
        horizontal=True
    )
    ...
    if processing_mode == "simple":
        ...
        processing_type = st.selectbox(
            "Choose processing effect:",
            ["grayscale", "edge_detection", "blur", "sharpen"],
            format_func=lambda x: {
                "grayscale": "🌑 Grayscale",
        ...
        with col1:
            st.download_button(
                ...
                data=pil_to_bytes(st.session_state.final_image),
        ...
        with col2:
            st.download_button(
                ...
                data=pil_to_bytes(st.session_state.processed_image),
                file_name=f"processed_{processing_type}.png",
                mime="image/png",
                use_container_width=True
            )
    ...
    else:  # Javanese processing
        st.info("🔄 Processing Javanese script with JawaLens 2.0 model...")
        ...
        try:
            ...
            with col4:
                ...
                # ============================================================
                st.download_button(
                    label="📦 Download All Processed Image (ZIP)",
                    data=zip_buffer,
